...
<?php
        ...
        //history
        $selectHistory = "SELECT `jobs_id`, `user_id`, `date` FROM `notification` WHERE user_id = '$row_userID' ORDER BY noti_id DESC";
        $selectAccept = "SELECT `user_host`, `user_regis`, `jobs_id`, `date` FROM `confirm` WHERE user_host = '$row_userID' ORDER BY confirm_id DESC ";
        $queryAcceptHistory = mysqli_query($connect,$selectAccept);
        $queryHistory = mysqli_query($connect,$selectHistory);
...
<style> 
    body{
          ...
          background-image: url(img/vector16.jpg);
...
<!-- Page content -->
<?php //fetch
    $queryCheckNoti = mysqli_query($connect,$Noti);
    $checkNoti = mysqli_fetch_assoc($queryCheckNoti); 
    $queryCheckAccept = mysqli_query($connect,$accept);
    $checkAccept = mysqli_fetch_assoc($queryCheckAccept); 
    mysqli_free_result($queryCheckNoti);
    mysqli_free_result($queryCheckAccept);
?>

<div class="w3-content" style="max-width:2000px;margin-top:40px">
    </div>
    <div class = "wrapMail">
    <span class = "MailHead"><i class="fas fa-history"></i></span>
    <h2>History</h2>
    <?php

    $num_row_queryAcceptHistory = mysqli_num_rows($queryAcceptHistory);
    $num_row_queryHistory = mysqli_num_rows($queryHistory);
    if($num_row_queryHistory != 0 || $num_row_queryAcceptHistory != 0){
       
        while($rows_checkNoti = mysqli_fetch_assoc($queryHistory)){
            $time = timestamp($rows_checkNoti['date']);?>
            <div class = "Mail"><a class ="Mailtext" href="PageNew.php?id=<?php echo $rows_checkNoti['jobs_id'];?>">
            You Have sent Resume <b>jobs_id : <?php echo $rows_checkNoti['jobs_id']?></b><br>
            <i class="fas fa-clock"></i>&nbsp;<?php echo $time?></a></div>
            
       <?php }

       while($rows_checkAccept = mysqli_fetch_assoc($queryAcceptHistory)){
            $time2 = timestamp($rows_checkAccept['date']);?>
            <div class = "Mail2"><a class = "Mailtext2" href="reciveResume.php?id=<?php echo $rows_checkAccept['user_regis'];?>
            &job = <?php echo $rows_checkAccept['jobs_id'];?>">
            You Has Accept Resume <b>jobs_id : <?php echo $rows_checkAccept['jobs_id'];?></b><br>
            <i class="fas fa-clock"></i>&nbsp;<?php echo $time2?></a></div>
      <?php }
    }  
    else{?>
        <span>Your not have History</span>
        <br><br>
    <?php } ?>
