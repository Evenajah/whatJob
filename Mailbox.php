...
<div class="w3-content" style="max-width:2000px;margin-top:40px">
    ...
    <div class = "wrapMail">
    <span class = "MailHead"><i class="fas fa-envelope-open"></i></span>
    <h2>MailBox</h2>
    <?php
    if($checkNoti || $checkAccept){
        mysqli_free_result($queryCheckNoti);
        mysqli_free_result($queryCheckAccept);
        $queryNotimail = mysqli_query($connect,$Noti);
        $queryAcceptmail = mysqli_query($connect,$accept);

        while($rows_checkNoti = mysqli_fetch_assoc($queryNotimail)){
            $time = timestamp($rows_checkNoti['date']);?>
            <div class = "Mail"><a class ="Mailtext" href="reciveResume.php?id=<?php echo $rows_checkNoti['user_id'];?>&job=<?php echo $rows_checkNoti['jobs_id'];?>">
            <b><?php echo $rows_checkNoti['username'];?></b> Have sent Resume <b>jobs_id : <?php echo $rows_checkNoti['jobs_id']?></b><br>
            <i class="fas fa-clock"></i>&nbsp;<?php echo $time?></a></div>
            
       <?php }

       while($rows_checkAccept = mysqli_fetch_assoc($queryAcceptmail)){
            $time2 = timestamp($rows_checkAccept['date']);?>
            <div class = "Mail2"><a class = "Mailtext2" href="agreement.php?id=<?php echo $rows_checkAccept['confirm_id'];?>">
            <b><?php echo $rows_checkAccept['username'];?></b>&nbsp;Has Accept Resume <b>jobs_id : <?php echo $rows_checkAccept['jobs_id'];?></b><br>
            <i class="fas fa-clock"></i>&nbsp;<?php echo $time2?></a></div>
      <?php }
    }  
    else{?>
        <span>Your not have Mail</span>
        <br><br>
    <?php } ?>
